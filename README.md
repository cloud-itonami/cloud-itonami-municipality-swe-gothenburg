# cloud-itonami-municipality-swe-gothenburg

Municipal-ordinance compliance catalog for **Gothenburg** (Göteborgs
Stad, Sweden) — the TWENTY-FOURTH municipality-level entry alongside
...
Sweden's national capital is Stockholm, not Gothenburg — Stockholm was
attempted in an earlier tick and abandoned when its ordinance PDF
rendered too font-subsetting-garbled to read even its own title.
Gothenburg's official PDFs rendered cleanly instead.
...
Both entries cite official [goteborg.se](https://goteborg.se/)-hosted
PDFs: the **Lokala ordningsföreskrifter för Göteborgs kommun** (local
public-order regulations; title and statutory basis directly confirmed
via PDF read, effective date 2025-01-01 WebSearch-corroborated from the
council's own record since the primary PDF exceeded the fetch size
limit) and the **Göteborgs Stadsmiljöpolicy** (City Environment Policy;
title and content directly confirmed, but its own adoption date is
deliberately omitted — the only date visible on the fetched pages
belongs to a quoted excerpt from a *different* document, and citing it
...
festivals and heritage sites for Gothenburg:
...
itself remains the City of Gothenburg's; this repo stores only
